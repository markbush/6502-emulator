...
final class BmiTests: XCTestCase {
  func testBmiNoBranch() {
    print("debug: testBmiNoBranch")
    let pins = Pins()
    let memory = TestHelper.initMemory(pins)
    let offset:UInt8 = 0x0a
    let target:UInt16 = TestHelper.RES_ADDR + UInt16(offset) + 2
    // First OP after reset is op
    memory[TestHelper.RES_ADDR] = TestHelper.BMI
    ...
    // Ensure negative clear
    cpu.status.value = 0

    // Next instruction should be op at RESET address
    XCTAssertEqual(pins.address.value, TestHelper.RES_ADDR)
    XCTAssertEqual(pins.data.value, TestHelper.BMI)

    print("debug: perform BMI")
    // decode OP - fetch offset
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.BMI)
  ...
  func testBmiBranchNoPageCross() {
    print("debug: testBmiBranchNoPageCross")
    let pins = Pins()
    let memory = TestHelper.initMemory(pins)
    let offset:UInt8 = 0x0a
    let target:UInt16 = TestHelper.RES_ADDR + UInt16(offset) + 2
  ...
  func testBmiBranchPageCross() {
    print("debug: testBmiBranchPageCross")
    let pins = Pins()
    let memory = TestHelper.initMemory(pins)
