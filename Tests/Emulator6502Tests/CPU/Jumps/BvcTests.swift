...
final class BvcTests: XCTestCase {
  func testBvcNoBranch() {
    print("debug: testBvcNoBranch")
    let pins = Pins()
    let memory = TestHelper.initMemory(pins)
    let offset:UInt8 = 0x0a
    let target:UInt16 = TestHelper.RES_ADDR + UInt16(offset) + 2
    // First OP after reset is op
    memory[TestHelper.RES_ADDR] = TestHelper.BVC
    ...
    XCTAssertEqual(pins.data.value, TestHelper.BVC)

    print("debug: perform BVC")
    // decode OP - fetch offset
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.BVC)
  ...
  func testBvcBranchNoPageCross() {
    print("debug: testBvcBranchNoPageCross")
    let pins = Pins()
    let memory = TestHelper.initMemory(pins)
    let offset:UInt8 = 0x0a
    let target:UInt16 = TestHelper.RES_ADDR + UInt16(offset) + 2
  ...
  func testBvcBranchPageCross() {
    print("debug: testBvcBranchPageCross")
    let pins = Pins()
    let memory = TestHelper.initMemory(pins)
