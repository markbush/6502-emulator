...
final class BplTests: XCTestCase {
  func testBplNoBranch() {
    ...
    XCTAssertEqual(pins.data.value, TestHelper.BPL)

    print("debug: perform BPL")
    // decode OP - fetch offset
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.BPL)
  ...
  func testBplBranchNoPageCross() {
    print("debug: testBplBranchNoPageCross")
    let pins = Pins()
    let memory = TestHelper.initMemory(pins)
    let offset:UInt8 = 0x0a
    let target:UInt16 = TestHelper.RES_ADDR + UInt16(offset) + 2
  ...
  func testBplBranchPageCross() {
    print("debug: testBplBranchPageCross")
    let pins = Pins()
    let memory = TestHelper.initMemory(pins)
