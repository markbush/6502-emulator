...
final class AdcZpXTests: XCTestCase {
  func testAdcZpXNoPageChange() {
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ADCZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ADCZpX)

    print("debug: perform ADC ZpX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ADCZpX)
