...
final class SbcAbsXTests: XCTestCase {
  func testSbcAbsXNoPageChange() {
    print("debug: testSbcAbsXNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.SBCAbsX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.SBCAbsX)

    print("debug: perform SBC AbsX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.SBCAbsX)
  ...
  func testSbcAbsXWithPageChange() {
    print("debug: testSbcAbsXWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
