...
final class DecZpXTests: XCTestCase {
  func testDecZpX() {
    print("debug: testDecZpX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.DECZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.DECZpX)

    print("debug: perform DEC")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.DECZpX)
