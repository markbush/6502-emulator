...
final class AdcAbsXTests: XCTestCase {
  func testAdcAbsXNoPageChange() {
    print("debug: testAdcAbsXNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ADCAbsX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ADCAbsX)

    print("debug: perform ADC AbsX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ADCAbsX)
  ...
  func testAdcAbsXWithPageChange() {
    print("debug: testAdcAbsXWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
