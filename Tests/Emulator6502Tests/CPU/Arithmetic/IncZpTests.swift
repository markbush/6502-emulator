...
final class IncZpTests: XCTestCase {
  func testIncZp() {
    print("debug: testIncZp")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.INCZp
    ...
    XCTAssertEqual(pins.data.value, TestHelper.INCZp)

    print("debug: perform INC")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.INCZp)
