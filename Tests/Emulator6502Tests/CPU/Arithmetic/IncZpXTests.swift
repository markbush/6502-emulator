...
final class IncZpXTests: XCTestCase {
  func testIncZpX() {
    print("debug: testIncZpX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.INCZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.INCZpX)

    print("debug: perform INC")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.INCZpX)
