...
final class SbcZpIndYTests: XCTestCase {
  func testSbcZpIndYNoPageChange() {
    print("debug: testSbcZpIndYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.SBCZpIndY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.SBCZpIndY)

    print("debug: perform SBC ZpIndY")
    // decode OP - fetch IAL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.SBCZpIndY)
  ...
  func testSbcZpIndYWithPageChange() {
    print("debug: testSbcZpIndYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
