...
final class DecAbsXTests: XCTestCase {
  func testDecAbsX() {
    print("debug: testDecAbsX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.DECAbsX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.DECAbsX)

    print("debug: perform DEC")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.DECAbsX)
