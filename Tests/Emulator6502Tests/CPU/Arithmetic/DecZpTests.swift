...
final class DecZpTests: XCTestCase {
  func testDecZp() {
    print("debug: testDecZp")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.DECZp
    ...
    XCTAssertEqual(pins.data.value, TestHelper.DECZp)

    print("debug: perform DEC")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.DECZp)
