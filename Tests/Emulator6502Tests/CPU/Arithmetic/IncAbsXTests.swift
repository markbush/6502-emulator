...
final class IncAbsXTests: XCTestCase {
  func testIncAbsX() {
    print("debug: testIncAbsX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.INCAbsX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.INCAbsX)

    print("debug: perform INC")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.INCAbsX)
