...
final class StaAbsYTests: XCTestCase {
  func testStaAbsY() {
    print("debug: testStaAbsY")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.STAAbsY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.STAAbsY)

    print("debug: perform STA")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.STAAbsY)
