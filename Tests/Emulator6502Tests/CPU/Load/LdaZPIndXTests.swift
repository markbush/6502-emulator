...
final class LdaZpIndXTests: XCTestCase {
  func testLdaZpIndX() {
    print("debug: testLdaZpIndX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.LDAZpIndX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.LDAZpIndX)

    print("debug: perform LDA ZpIndX")
    // decode OP - fetch BAL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.LDAZpIndX)
