...
final class LdaZpXTests: XCTestCase {
  func testLdaZpXNoPageChange() {
    print("debug: testLdaZpXNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.LDAZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.LDAZpX)

    print("debug: perform LDA ZpX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.LDAZpX)
  ...
  func testLdaZpXWithPageChange() {
    print("debug: testLdaZpXWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
