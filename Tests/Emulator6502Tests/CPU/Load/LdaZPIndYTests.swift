...
final class LdaZpIndYTests: XCTestCase {
  func testLdaZpIndYNoPageChange() {
    print("debug: testLdaZpIndYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.LDAZpIndY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.LDAZpIndY)

    print("debug: perform LDA ZpIndY")
    // decode OP - fetch IAL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.LDAZpIndY)
  ...
  func testLdaZpIndYWithPageChange() {
    print("debug: testLdaZpIndYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
