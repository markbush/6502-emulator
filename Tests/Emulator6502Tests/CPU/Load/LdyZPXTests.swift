...
final class LdyZpXTests: XCTestCase {
  func testLdyZpXNoPageChange() {
    print("debug: testLdyZpXNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.LDYZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.LDYZpX)

    print("debug: perform LDY ZpX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.LDYZpX)
  ...
  func testLdyZpXWithPageChange() {
    print("debug: testLdyZpXWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
