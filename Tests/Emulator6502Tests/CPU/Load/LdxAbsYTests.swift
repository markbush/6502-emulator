...
final class LdxAbsYTests: XCTestCase {
  func testLdxAbsYNoPageChange() {
    print("debug: testLdxAbsYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.LDXAbsY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.LDXAbsY)

    print("debug: perform LDX AbsY")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.LDXAbsY)
  ...
  func testLdxAbsYWithPageChange() {
    print("debug: testLdxAbsYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
