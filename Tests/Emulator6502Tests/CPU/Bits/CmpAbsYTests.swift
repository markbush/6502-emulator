...
final class CmpAbsYTests: XCTestCase {
  func testCmpAbsYNoPageChange() {
    print("debug: testCmpAbsYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.CMPAbsY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.CMPAbsY)

    print("debug: perform CMP AbsY")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.CMPAbsY)
  ...
  func testCmpAbsYWithPageChange() {
    print("debug: testCmpAbsYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
