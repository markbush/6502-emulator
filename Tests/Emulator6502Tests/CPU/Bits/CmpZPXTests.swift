...
final class CmpZpXTests: XCTestCase {
  func testCmpZpXNoPageChange() {
    print("debug: testCmpZpXNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.CMPZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.CMPZpX)

    print("debug: perform CMP ZpX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.CMPZpX)
  ...
  func testCmpZpXWithPageChange() {
    print("debug: testCmpZpXWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
