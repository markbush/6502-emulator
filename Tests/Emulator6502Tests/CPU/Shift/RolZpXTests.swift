...
final class RolZpXTests: XCTestCase {
  func testRolZpX() {
    print("debug: testRolZpX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ROLZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ROLZpX)

    print("debug: perform ROL")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ROLZpX)
