...
final class AslZpTests: XCTestCase {
  func testAslZp() {
    print("debug: testAslZp")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ASLZp
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ASLZp)

    print("debug: perform ASL")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ASLZp)
