...
final class LsrZpXTests: XCTestCase {
  func testLsrZpX() {
    print("debug: testLsrZpX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.LSRZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.LSRZpX)

    print("debug: perform LSR")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.LSRZpX)
