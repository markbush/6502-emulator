...
final class RolAbsXTests: XCTestCase {
  func testRolAbsX() {
    print("debug: testRolAbsX")
    let pins = Pins()
    let testValue1:UInt8 = 0xc3
    let testValue2:UInt8 = 0x26
    let memory = TestHelper.initMemory(pins)
    let memStore:UInt16 = 0x5a3c
    let offset:UInt8 = 0x05
    let actualStore:UInt16 = 0x5a41
    // First OP after reset is op
    memory[TestHelper.RES_ADDR] = TestHelper.ROLAbsX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ROLAbsX)

    print("debug: perform ROL")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ROLAbsX)
