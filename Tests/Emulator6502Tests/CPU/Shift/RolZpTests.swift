...
final class RolZpTests: XCTestCase {
  func testRolZp() {
    print("debug: testRolZp")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ROLZp
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ROLZp)

    print("debug: perform ROL")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ROLZp)
