...
final class AslAbsXTests: XCTestCase {
  func testAslAbsX() {
    print("debug: testAslAbsX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ASLAbsX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ASLAbsX)

    print("debug: perform ASL")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ASLAbsX)
