...
final class AslAbsTests: XCTestCase {
  func testAslAbs() {
    print("debug: testAslAbs")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ASLAbs
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ASLAbs)

    print("debug: perform ASL")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ASLAbs)
