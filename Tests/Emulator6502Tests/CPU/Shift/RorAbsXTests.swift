...
final class RorAbsXTests: XCTestCase {
  func testRorAbsX() {
    print("debug: testRorAbsX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.RORAbsX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.RORAbsX)

    print("debug: perform ROR")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.RORAbsX)
