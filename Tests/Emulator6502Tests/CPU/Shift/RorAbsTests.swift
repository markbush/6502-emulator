...
final class RorAbsTests: XCTestCase {
  func testRorAbs() {
    print("debug: testRorAbs")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.RORAbs
    ...
    XCTAssertEqual(pins.data.value, TestHelper.RORAbs)

    print("debug: perform ROR")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.RORAbs)
