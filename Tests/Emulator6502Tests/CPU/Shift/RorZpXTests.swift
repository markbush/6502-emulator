...
final class RorZpXTests: XCTestCase {
  func testRorZpX() {
    print("debug: testRorZpX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.RORZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.RORZpX)

    print("debug: perform ROR")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.RORZpX)
