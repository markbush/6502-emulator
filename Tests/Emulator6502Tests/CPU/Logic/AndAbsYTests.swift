...
final class AndAbsYTests: XCTestCase {
  func testAndAbsYNoPageChange() {
    print("debug: testAndAbsYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ANDAbsY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ANDAbsY)

    print("debug: perform AND AbsY")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ANDAbsY)
  ...
  func testAndAbsYWithPageChange() {
    print("debug: testAndAbsYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
