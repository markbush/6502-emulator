...
final class OraZpIndXTests: XCTestCase {
  func testOraZpIndX() {
    print("debug: testOraZpIndX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ORAZpIndX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ORAZpIndX)

    print("debug: perform ORA ZpIndX")
    // decode OP - fetch BAL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ORAZpIndX)
