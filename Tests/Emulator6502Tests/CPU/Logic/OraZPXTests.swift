...
final class OraZpXTests: XCTestCase {
  func testOraZpXNoPageChange() {
    print("debug: testOraZpXNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ORAZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ORAZpX)

    print("debug: perform ORA ZpX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ORAZpX)
  ...
  func testOraZpXWithPageChange() {
    print("debug: testOraZpXWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
