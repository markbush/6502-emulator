...
final class OraAbsXTests: XCTestCase {
  func testOraAbsXNoPageChange() {
    print("debug: testOraAbsXNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ORAAbsX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ORAAbsX)

    print("debug: perform ORA AbsX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ORAAbsX)
  ...
  func testOraAbsXWithPageChange() {
    print("debug: testOraAbsXWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
