...
final class EorAbsYTests: XCTestCase {
  func testEorAbsYNoPageChange() {
    print("debug: testEorAbsYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.EORAbsY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.EORAbsY)

    print("debug: perform EOR AbsY")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.EORAbsY)
  ...
  func testEorAbsYWithPageChange() {
    print("debug: testEorAbsYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
