...
final class OraAbsYTests: XCTestCase {
  func testOraAbsYNoPageChange() {
    print("debug: testOraAbsYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ORAAbsY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ORAAbsY)

    print("debug: perform ORA AbsY")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ORAAbsY)
  ...
  func testOraAbsYWithPageChange() {
    print("debug: testOraAbsYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
