...
final class OraZpIndYTests: XCTestCase {
  func testOraZpIndYNoPageChange() {
    print("debug: testOraZpIndYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ORAZpIndY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ORAZpIndY)

    print("debug: perform ORA ZpIndY")
    // decode OP - fetch IAL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ORAZpIndY)
  ...
  func testOraZpIndYWithPageChange() {
    print("debug: testOraZpIndYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
