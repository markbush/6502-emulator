...
final class AndZpIndXTests: XCTestCase {
  func testAndZpIndX() {
    print("debug: testAndZpIndX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ANDZpIndX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ANDZpIndX)

    print("debug: perform AND ZpIndX")
    // decode OP - fetch BAL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ANDZpIndX)
