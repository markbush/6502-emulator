...
final class EorZpIndXTests: XCTestCase {
  func testEorZpIndX() {
    print("debug: testEorZpIndX")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.EORZpIndX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.EORZpIndX)

    print("debug: perform EOR ZpIndX")
    // decode OP - fetch BAL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.EORZpIndX)
