...
final class AndZpIndYTests: XCTestCase {
  func testAndZpIndYNoPageChange() {
    print("debug: testAndZpIndYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ANDZpIndY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ANDZpIndY)

    print("debug: perform AND ZpIndY")
    // decode OP - fetch IAL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ANDZpIndY)
  ...
  func testAndZpIndYWithPageChange() {
    print("debug: testAndZpIndYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
