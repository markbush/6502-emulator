...
final class AndAbsXTests: XCTestCase {
  func testAndAbsXNoPageChange() {
    print("debug: testAndAbsXNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.ANDAbsX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.ANDAbsX)

    print("debug: perform AND AbsX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.ANDAbsX)
  ...
  func testAndAbsXWithPageChange() {
    print("debug: testAndAbsXWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
