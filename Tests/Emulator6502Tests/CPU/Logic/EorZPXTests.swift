...
final class EorZpXTests: XCTestCase {
  func testEorZpXNoPageChange() {
    print("debug: testEorZpXNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.EORZpX
    ...
    XCTAssertEqual(pins.data.value, TestHelper.EORZpX)

    print("debug: perform EOR ZpX")
    // decode OP - fetch ADL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.EORZpX)
  ...
  func testEorZpXWithPageChange() {
    print("debug: testEorZpXWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
