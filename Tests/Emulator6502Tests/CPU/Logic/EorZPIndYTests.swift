...
final class EorZpIndYTests: XCTestCase {
  func testEorZpIndYNoPageChange() {
    print("debug: testEorZpIndYNoPageChange")
    let pins = Pins()
    ...
    memory[TestHelper.RES_ADDR] = TestHelper.EORZpIndY
    ...
    XCTAssertEqual(pins.data.value, TestHelper.EORZpIndY)

    print("debug: perform EOR ZpIndY")
    // decode OP - fetch IAL
    TestHelper.cycle(cpu, pins: pins, mem: memory)
    XCTAssertEqual(cpu.ir.value, TestHelper.EORZpIndY)
  ...
  func testEorZpIndYWithPageChange() {
    print("debug: testEorZpIndYWithPageChange")
    let pins = Pins()
    let testValue1:UInt8 = 0x26
    let testValue2:UInt8 = 0x1c
    let memory = TestHelper.initMemory(pins)
