public class Register8 : Bus8 {
  public func shiftLeft() -> (UInt8,Bool,Bool,Bool) {
  ...
  public func shiftRight() -> (UInt8,Bool,Bool,Bool) {
  ...
  public func rotateLeft(carryIn: Bool) -> (UInt8,Bool,Bool,Bool) {
    let carryOut = (value & 0b1000_0000) == 0b1000_0000
  ...
  public func rotateRight(carryIn: Bool) -> (UInt8,Bool,Bool,Bool) {
    let carryOut = (value & 0b0000_0001) == 1
    var newValue = value >> 1
  ...
  public func adc(_ from: UInt8, carryIn: Bool) -> (UInt8,Bool,Bool,Bool,Bool) {
  ...
  public func and(_ from: UInt8) -> (UInt8,Bool,Bool) {
    let result = value & from
    let negative = (result & 0x80) == 0x80
    return (result, negative, result == 0)
  }
  public func eor(_ from: UInt8) -> (UInt8,Bool,Bool) {
    let result = value ^ from
    let negative = (result & 0x80) == 0x80
    return (result, negative, result == 0)
  }
  public func or(_ from: UInt8) -> (UInt8,Bool,Bool) {
    let result = value | from
    let negative = (result & 0x80) == 0x80
    return (result, negative, result == 0)
  }
